...
async function runNativeElectronTests () {
  ...
  const firstArg = unknownArgs.slice(2)[0]
  let singleTestTarget
  if (firstArg && !TEST_TARGETS.includes(firstArg)) {
    console.log(`${fail} ${firstArg} must be a subset of [${[TEST_TARGETS].join(', ')}]`)
    process.exit(1)
  } else {
    singleTestTarget = firstArg
  }

  // If a specific target was passed, only build and run that target
  if (singleTestTarget) {
    console.info('\nRunning native test for target:', singleTestTarget)

    // Build single test target
    const build = childProcess.spawnSync('ninja', ['-C', outDir, firstArg], {
      cwd: path.resolve(__dirname, '../..'),
      stdio: 'inherit'
    })
    ...
    if (build.status !== 0) {
      console.log(`${fail} ${singleTestTarget} failed to build.`)
      process.exit(1)
    }

    // Run single test target
    const run = childProcess.spawnSync(`./${outDir}/${singleTestTarget}`, {
      cwd: path.resolve(__dirname, '../..'),
      stdio: 'inherit'
    })

    if (run.status !== 0) {
      console.log(`${fail} ${singleTestTarget} test failed.`)
      process.exit(1)
    }
  } else {
